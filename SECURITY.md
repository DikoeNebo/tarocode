...
Security fixes target the latest GitHub Release of Keycode on Windows.

## What to know before enabling features

- **Cursor CDP (`--remote-debugging-port`)** — anything on this PC that can reach `127.0.0.1:9222` can control Cursor. Use the Keycode shortcut when you need background paste; quit Cursor with debugging when you do not.
...
- **Logs** — Keycode does not log card prompts or chat text. Do not paste secrets into Issues.
